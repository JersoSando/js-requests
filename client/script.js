...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/Books-Are-Awsome')
    .then(res => {
        console.log(res.data)
        let repeatTextBtn = document.getElementById('repeat-text')
        console.log(repeatTextBtn)
        repeatTextBtn.textContent = res.data
...
const queryReq = () => {
    axios.get('http://localhost:3000/?besttvshow=theoffice')
    .then(res => {
        console.log(res.data)
    })
}
...
function createFood () {
    ...
    .then(function(){
        console.log(res.data);
        res.data.textContent = document.querySelector('input')
    })
}